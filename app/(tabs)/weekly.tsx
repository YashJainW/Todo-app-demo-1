...
export default function WeeklyScreen() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("user_id", user.id)
        .eq("type", "Weekly")
        .gte("timeframe->>startDate", startStr)
        .lte("timeframe->>endDate", endStr)
  ...
  const toggleTaskCompletion = async (task: Task) => {
    try {
      ...
      const progress = await loadChildrenProgress(
        (tasks.length ? tasks : [task]).map((t) => t.id)
      );
      setChildrenProgress(progress);
    } catch (error) {
    ...
    >
      <View style={styles.taskContent}>
        ...
        <View style={styles.taskText}>
          ...
          {task.timeframe.startDate && task.timeframe.endDate && (
            <Text style={styles.taskDate}>
              {new Date(task.timeframe.startDate).toLocaleDateString()} -{" "}
              {new Date(task.timeframe.endDate).toLocaleDateString()}
            </Text>
          )}
        </View>
        <TouchableOpacity
  ...
  const formatWeekRange = (date: Date) => {
    ...
    const { start: currentStart, end: currentEnd } = getWeekRange(today);

    if (start.toDateString() === currentStart.toDateString()) {
      return "This Week";
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.weekContainer}>
          ...
          <Text style={styles.weekDates}>{getWeekDates(selectedWeek)}</Text>
        </View>
      ...
      <View style={styles.tasksContainer}>
        <Text style={styles.sectionTitle}>
          {tasks.length > 0
            ? `${tasks.length} Weekly Goal${tasks.length !== 1 ? "s" : ""}`
            : "No Weekly Goals"}
        </Text>
        ...
        {tasks.length > 0 ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              Add a weekly goal for{" "}
              {formatWeekRange(selectedWeek).toLowerCase()} to get started
            </Text>
          </View>
        )}
      </View>
